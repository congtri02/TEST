                        ...
                        <div class="accordion-item border mx-2 mb-2">
                            <div id="heading{{ $key }}">
                                <h5 class="mb-0 position-relative">
                                    ...
                                    <span class="question-edit-icons position-absolute"
                                          style="{{ lang() == 'en' ? 'right: 20px' : 'left: 20px' }}">
                                        <a href="{{ route('question.edit', $question->id) }}" class=""><i
                                        ...
                                        <form action="{{ route('question.destroy', $question->id) }}" method="POST"
                                            ...
                                            <button type="submit" class="border-0 bg-white"
                                                    onclick="return confirm('{{ trans('admin.alert_delete_msg') }}')">
                                    ...
                                    <ul class="list-unstyled">
                                        @foreach($question->answers as $key => $answer)
                                            <li class="{{ lang() == 'ar' ? 'mr-4' : 'ml-4' }} {{ $answer->status ? 'text-success' : 'text-danger' }}">
                                                <span>{{ trans('admin.answer') . ' ' . ($key + 1) }} : </span>
                                                {{ $answer->title }}
                                            </li>
                                        @endforeach
                                    </ul>

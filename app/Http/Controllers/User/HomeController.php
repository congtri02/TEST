...
class HomeController extends Controller
{
    ...
    {
        $exam = Exam::all()->where('isActive', True)->first();
        return view('style.take_quiz', compact('exam'));
    }
    public function contentQuiz(Request $request)
    {
        $quizResults = [];
        $quizzes = Quiz::all();
        ...
        return view('style.content_quiz', compact('quizResults', 'quizzes'));
    }
    ...
    public function copy(Request $request)
    {
        ...
        $quizContent = $request->input('quizContent');

        $exam = CopyCount::firstOrCreate(
            ['template' => $template],
            ['action' => 0]
        );

        $exam->increment('action');
        $exam->quizContent = $quizContent;
        $exam->save();

        return response()->json(['message' => 'Copied to clipboard!', 'copy_count' => $exam->action]);
    }
    ...
    {
        $exams = Exam::all();
        return view('style.exam', compact('exams'));
    }
